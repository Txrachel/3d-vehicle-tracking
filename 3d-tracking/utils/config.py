...
# KITTI tracking
# cfg.KITTI.TRACKING = edict()  # easy dict
...
cfg.OWN_DATA_PATH = '/harddisk_1/data_center/'
cfg.KITTI.TRACKING = edict()  # easy dict
cfg.KITTI.TRACKING.SAVE_PATH = '/home/ubuntu/3d_tracking/3d-tracking/data/'
cfg.KITTI.TRACKING.CLASS_LIST = ['Car', 'Van', 'Truck']
cfg.KITTI.TRACKING.PATH = join(cfg.OWN_DATA_PATH, 'kitti_tracking')
cfg.KITTI.TRACKING.IM_PATH = join(cfg.KITTI.TRACKING.PATH, 'data_tracking_image_2/', 'training', 'image_02')
cfg.KITTI.TRACKING.LABEL_PATH = join(cfg.KITTI.TRACKING.PATH, 'data_tracking_label_2/'
                                     'training', 'label_02')
cfg.KITTI.TRACKING.PRED_PATH = join(cfg.OUTPUT_PATH, 'kitti_tracking', 'training', 'pred_02')
cfg.KITTI.TRACKING.PKL_FILE = join(cfg.KITTI.TRACKING.SAVE_PATH,
                                   'kitti_train_trk_labels.pkl')
cfg.KITTI.TRACKING.OXT_PATH = join(cfg.KITTI.TRACKING.PATH,  'oxts', 'training', 'oxts')
cfg.KITTI.TRACKING.CALI_PATH = join(cfg.KITTI.TRACKING.PATH, 'calib', 'training', 'calib')


# not in gen dataset.py
tracking_vid_list = [str(num).zfill(4) for num in range(21)]
cfg.KITTI.TRACKING.TRAIN_SPLIT = tracking_vid_list[:15]
cfg.KITTI.TRACKING.VAL_SPLIT = tracking_vid_list[15:]  # 前14个做train
