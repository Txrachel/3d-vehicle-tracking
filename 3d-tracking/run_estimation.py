...
SAVE_PATH = 'output/{SESS}_{EP}_{SET}_{SPLIT}_set/'.format(  # 616_030_kitti_train_set
...
def gen_3d_output():
    ...
    for i in range(len(JSON_PATHS) // m + 1):
        for JSON, GPU in zip(JSON_PATHS[m * i:m * i + m], GPUS * args.n_tasks):
            ...
            print(i, GPU, cmd)
            if not args.dry_run:
                if not args.overwrite and os.path.isfile(os.path.join('output', TRK)): # yes and no,首次运行会生成
                    print("SKIP running. Generated file {} Found".format(TRK))
                    continue
                subprocess_env = os.environ.copy()
                subprocess_env['CUDA_VISIBLE_DEVICES'] = GPU
                p = subprocess.Popen(cmd, shell=True, env=subprocess_env)  # here start estimating
                ps.append(p)
                sleep(1)
        if not args.dry_run:
            for p in ps:
