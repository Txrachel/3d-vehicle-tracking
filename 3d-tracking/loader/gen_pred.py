...
if args.set == 'gta':
    ...
    if args.kitti_task == 'detect':
        ...
        det_name = os.path.join(DATASET.SAVE_PATH,  # DATASET.PATH,
                            'kitti_{}_trk_detections.pkl'.format(
                                args.split))
        save_name = os.path.join(DATASET.SAVE_PATH,  # DATASET.PATH,
...
class Dataset():
    ...
    def __init__(self):
        ...
        if self.set == 'kitti':  ##
            DATASET.LABEL_PATH = DATASET.SAVE_PATH

        pattern = 'final.json' if args.set == 'gta' else '.json'
        self.data_path = load_label_path(DATASET.LABEL_PATH, pattern)  # extract name of .json
