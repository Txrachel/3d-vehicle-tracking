...
'''
python loader/gen_dataset.py kitti train   // kitti-tracking/training
...
if args.set == 'gta':
    ...
    if args.kitti_task == 'detect':
        ...
        DATASET.PKL_FILE = os.path.join(DATASET.SAVE_PATH,  # 下载的后面带了rrc后缀  #.PATH
                            'kitti_{}_trk_labels.pkl'.format(
                                args.split))
        det_name = os.path.join(DATASET.SAVE_PATH, #.PATH
                            'kitti_{}_trk_detections.pkl'.format(  # 这个文件是数据集本身提供的吗
                                args.split))
        save_name = os.path.join(DATASET.SAVE_PATH, #.PATH
...
class Dataset():
    def __init__(self):
        ...
        self.split = args.split  # default train // train val test
        self.mode = args.mode  # default test // train test
        self.kitti_task = args.kitti_task  # default track // track detect

        # load data
        if args.load_cache:  # default False  在运行过后可以加载cache的pkl文件
            print("Load pickle file: {}".format(DATASET.PKL_FILE))
            self.data_label = load_label_pickle(DATASET)
        else:
            print("Load label file from path: {}".format(DATASET.LABEL_PATH))  # 上面定义
            if args.set == 'gta':
        ...
        for d_idx, data_label in enumerate(self.data_label):
            det_ = det_result[st:st + data_len[d_idx]] if args.is_pred else None  # NONE
            data_idx, box_idx, det_res = self.clean_idx(d_idx, list(data_label),  # is_pred=false // det_rets = []
                                                        det_)
            st += data_len[d_idx]  # det_
            self.data_idx.append(data_idx)
            self.box_idx.append(box_idx)
            self.det_result.append(det_res)
        self.data_len = [len(n) for n in self.data_idx]  # after filtering
        print(
    ...
    def kitti_detection_label(self):
        ...
        print("pkl file : ", DATASET.PKL_FILE)
        with open(os.path.join(DATASET.PKL_FILE), 'wb') as f:
            pickle.dump(data_label_list, f, -1)
        return data_label_list
    ...
    def kitti_tracking_label(self):
        ...
        for n in range(len([ip[0] for ip in os.walk(DATASET.IM_PATH)]) - 1):  # return root dirs files  //sequence
            fields = tu.read_oxts(DATASET.OXT_PATH, n)  # oxts gps/imu info

            poses = [tu.KittiPoseParser(fields[i]) for i in range(len(fields))]
            projection = tu.read_calib(n, calib_dir=DATASET.CALI_PATH)  # calib
            ...
            for fr, fname in enumerate(file_list):
                ...
                position = poses[fr].position - poses[0].position  # relative | important
                obj_dict = {}
                obj_dict['class'] = []
                obj_dict['truncated'] = []
                obj_dict['occluded'] = []
                ...
                if os.path.isfile(label_file):
                    ...
                    with open(label_file, 'r') as f:
                        for line in f:
                            ...
                            obj_dict['num_boxes'].append(num_lines)
                            obj_dict['pixel'].append(args.min_pixel + 1)
                data_label[obj_dict['img_name']] = obj_dict  # every image name correspond to one obj_dict recording all
                                                             # label info

            data_label_list.append(data_label)
        with open(os.path.join(DATASET.PKL_FILE), 'wb') as f:  # output pkl file
            pickle.dump(data_label_list, f, -1)
        return data_label_list
    ...
    def get_data2bdd(self, data_idx, data_label, box_idx, det_result):
        ...
        for idx, im_path in enumerate(data_idx):
            ...
            if idx % args.verbose_interval == 0 and idx != 0:  # print every 10 imgs
            ...
            if args.is_pred:
                ...
                filename = os.path.join(
                                DATASET.SAVE_PATH,  # DATASET.LABEL_PATH,
                                os.path.splitext(im_path)[0] + '.json'
                                )
                bh.dump_json(filename, frame_data)
    ...
    def get_data_from_id_bdd(self, obj, check_avail):
        ...
        for idx in check_avail:
            ...
            if args.set == 'kitti':
                ...
                # with GTA (center of 3D box)  3d center position diff
    ...
    def clean_idx(self, d_idx, data_key, det_result):
        ...
        print("Sequence {}: Data length: {}, Valid data length: {}".format(
            d_idx,
            len(data_key),
            len(valid_key_list)))  # is_pred==False vaild_det_list=[]
    ...
    def is_valid_idx(self, d_idx, key):
        ...
        for j in range(len(obj['class'])):
            if (obj['class'][j] in DATASET.CLASS_LIST \
                ...
                is_valid.append(j)
        return np.array(is_valid)  # check validity of data
